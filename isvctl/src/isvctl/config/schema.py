...
class PlatformCommands(BaseModel):
    ...
    Groups commands for a platform (kubernetes, slurm, bare_metal, network, vm, iam, iso).
...
class ValidationConfig(BaseModel):
    ...
    platform: str | None = Field(
        default=None, description="Platform: kubernetes, slurm, bare_metal, network, vm, iam, iso"
    )
...
class RunConfig(BaseModel):
    ...
    commands: dict[str, PlatformCommands] = Field(
        default_factory=dict,
        description="Lifecycle commands by platform (kubernetes, slurm, bare_metal, network, vm, iam, iso)",
    )
